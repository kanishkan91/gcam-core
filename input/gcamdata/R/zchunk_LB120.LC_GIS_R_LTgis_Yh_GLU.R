...
module_aglu_LB120.LC_GIS_R_LTgis_Yh_GLU <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    Various_CarbonData_LTsage <- get_data(all_data,"aglu/Various_CarbonData_LTsage") %>%
      filter(variable =="mature age") %>%
      rename(`mature age`=value) %>%
      select(LT_SAGE,`mature age`)
    ...
    L100.Ref_veg_carbon_Mg_per_ha %>%
      ...
      rename(soil_c = `soil_c (0-100 cms)`, vegc_ag = `veg_c (above ground biomass)`, vegc_bg = `veg_c (below ground biomass)`) %>%
    ...
    L100.Land_type_area_ha %>%
      ...
      distinct() %>%
      mutate(`mature age` = if_else(is.na(`mature age`),1,`mature age`))->Land_for_carbon
    ...
    # Pasture carbon is the same as grasslnad carbon values. This is similar to our previous structure where we use the Houghton values directly.
    L120.LC_soil_veg_carbon_GLU_all_cat %>%
      ...
      mutate(Land_Type = aglu.PASTURE_NODE_NAMES,
             soil_c = if_else(is.na(soil_c), aglu.DEFAULT_SOIL_CARBON_PASTURE,if_else(soil_c==0,aglu.DEFAULT_SOIL_CARBON_PASTURE,soil_c)),
             veg_c = if_else(is.na(veg_c), aglu.DEFAULT_VEG_CARBON_PASTURE,if_else(veg_c==0,aglu.DEFAULT_VEG_CARBON_PASTURE,veg_c)),
             `mature age` = aglu.DEFAULT_MATURITY_AGE_PASTURE)->L120.LC_soil_veg_carbon_GLU_pasture


    # Note that we set the default maturity age for Crop Land and Urban Land to 1 based on Houghton values.
    L120.LC_soil_veg_carbon_GLU_pasture %>%
      mutate(Land_Type = paste0("Cropland"),soil_c = aglu.DEFAULT_SOIL_CARBON_CROPLAND, veg_c = aglu.DEFAULT_VEG_CARBON_CROPLAND, `mature age`= 1)->L120.LC_soil_veg_carbon_GLU_crop
    ...
    L120.LC_soil_veg_carbon_GLU %>%
      ...
      add_units("kg/m2") %>%
      add_comments("Land types from SAGE, HYDE, WDPA merged and reconciled; missing zeroes backfilled; interpolated to AGLU land cover years. Soil carbon is at a depth of 0-100 cms and vegetation carbon is a combination of above and below ground biomass.") %>%
